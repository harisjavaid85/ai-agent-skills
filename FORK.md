...
- **`.agents/install-block.md` describes upstream's install story, not this fork's.** It is imported verbatim and is **not** a source of truth here: it names `mattpocock-skills` in Claude Code's official marketplace, and `skills.sh/mattpocock/skills`. This fork is in neither. The fork's canonical install wording lives in the top-level `README.md`. Two of its claims are load-bearing elsewhere and are false here: that `docs/` pages omit install commands because ai-hero renders a widget above the body (this fork has no such widget, so its pages carry a Quickstart block), and that `setup-matt-pocock-skills` is the skill to select on install (here it is `setup-repo-skills`). Rewrite or delete it before citing it.
