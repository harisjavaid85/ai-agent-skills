...
- **PRD** and **Issues completed** sections appear only when a `<slug>` is provided.
- **Notes** appears only when there's a genuine callout — out-of-scope / follow-ups, a known limitation or risk, a deliberate deviation from the linked plan/PRD, or a migration/deploy caveat. Terse bullets; omit the section otherwise.
...
<lists the subjects from `git log <base>..HEAD`, verbatim with their tags>
