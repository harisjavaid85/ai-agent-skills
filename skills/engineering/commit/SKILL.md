...
2. **Group & tag.** Whole files only. Prefer **one tag per commit** — split a multi-category change into separate one-tag commits. When a commit _unavoidably_ spans two categories — two coupled files that won't build or pass tests apart, or one file whose two categories whole-file staging can't separate — use `[Tag1][Tag2]` (dominant tag first, never three). The resulting grouping, including any two-tag commit, is shown in the plan in both modes.
