...
Fill in one value per group; the file explains each. It must stay gitignored — `.to-done/.gitignore` is where that rule belongs, so it travels with the directory. Nothing is written into the image. The loop startup reports what each provider resolved to.
...
It is assigned to the token owner either way. Repeat any "missing" line the loop prints — a draft PR with no label looks exactly like one still being worked on.
