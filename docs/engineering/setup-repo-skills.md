...
The defining constraint is the authority split: this skill owns only **structural** setup — what's derivable from disk and convention. It never decides the repo's domain layout (single- vs multi-context); that judgment belongs to [bootstrap-context](https://aihero.dev/skills-bootstrap-context).

## When to reach for it

Type `/setup-repo-skills`, or the agent reaches for it automatically when a task fits.

Reach for it once per repo, before the issue-lifecycle skills — [to-spec](https://aihero.dev/skills-to-spec), [to-tickets](https://aihero.dev/skills-to-tickets), and [triage](https://aihero.dev/skills-triage) stop and ask for it when their config is missing. It walks you through one section at a time, leading with the recommended answer: repo mode, issue tracker (GitHub, GitLab, or local markdown), label vocabularies, verify tiers, coding standards.
...
The **run-once keystone** of the whole system — every other engineering skill assumes its output. Follow it with [bootstrap-context](https://aihero.dev/skills-bootstrap-context) to establish the domain glossary. [ask-matt](https://aihero.dev/skills-ask-matt) routes the wider system.
